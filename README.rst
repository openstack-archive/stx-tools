...
required to download a group of RPMs, SRPMs, source code packages etc.
These tools live inside ``centos-mirror-tools`` directory.

.. code-block :: bash
    $ cd centos-mirror-tools

All stuff included in this directory must be visble inside the container
...
StarlingX ISO image creation required some customized packages, in this
step a set of patches a customizations are applied to the source code
in order to become in RPMs packages. we have an script called ``tb.sh``
