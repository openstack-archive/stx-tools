...
def take_snapshot(labname, snapshot_name, socks=None):
    ...
    LOG.info("#### Taking snapshot {} of lab {}".format(snapshot_name, labname))
    LOG.info("VMs in lab {}: {}".format(labname, vms))
    LOG.info("VMs running in lab {}: {}".format(labname, runningvms))
    ...
    if runningvms:
        ...
        while retry < 20:
            LOG.info("Waiting for VMs to come up running after taking snapshot..."
                     "Up VMs are {} ".format(new_vms))
            if len(runningvms) < len(new_vms):
                time.sleep(1)
                new_vms = get_all_vms(labname, option="runningvms")
                retry += 1
            else:
                LOG.info("All VMs {} are up running after taking snapshot...".format(vms))
                break
...
def vboxmanage_createvm(hostname, labname):
    ...
    result = subprocess.check_output(['vboxmanage', 'createvm', '--name', hostname, '--register', '--ostype',
                                      'Linux_64', '--groups', group], stderr=subprocess.STDOUT)
...
def vboxmanage_modifyvm(hostname=None, cpus=None, memory=None, nic=None, nictype=None, nicpromisc=None, nicnum=None,
                        intnet=None, hostonlyadapter=None, natnetwork=None, uartbase=None, uartport=None,
...
def vboxmanage_port_forward(hostname, network, local_port, guest_port, guest_ip):
    ...
    LOG.info("Removing previous forwarding rule '{}' from NAT netowrk '{}'".format(rule_name, network))
    cmd = ['vboxmanage', 'natnetwork', 'modify', '--netname', network, '--port-forward-4', 'delete', rule_name]
    try:
        result = subprocess.check_output(cmd, stderr=subprocess.STDOUT)
    ...
    LOG.info("Updating port-forwarding rule to: {}".format(rule))
...
def vboxmanage_deletemedium(hostname, vbox_home_dir='/home'):
    ...
    for disk in disk_list:
        LOG.info("Disconnecting disk {} from vbox.".format(disk))
        try:
            ...
            LOG.info("Error disconnecting disk, continuing. "
                     "Details: stdout: {} stderr: {}".format(e.stdout, e.stderr))
        LOG.info("Removing backing file {}".format(disk))
        try:
...
def vboxmanage_createmedium(hostname=None, disk_list=None, vbox_home_dir='/home'):
    ...
    for disk in disk_list:
        ...
        try:
            ...
            LOG.info("Error stdout: {} stderr: {}".format(e.stdout, e.stderr))
            raise
...
def vboxmanage_startvm(hostname=None, force=False):
    ...
    LOG.info("VM '{}' started.".format(hostname))
