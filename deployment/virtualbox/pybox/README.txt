...
The automated installer provides you an easy tool to install StarlingX
AIO-SX, AIO-DX, Standard and Storage setups on Linux hosts on Virtualbox
5.1.x.

The main concept of the autoinstaller is the stage and the chain. A stage
is an atomic set of actions taken by the autoinstaller. A chain is a set
of stages executed in a specific order. Stages can be executted
intependently and repeted as many time as user needs. Chains can be
configured with the desired stages by the user. Or the user can select a
specific chain from the available one.
...
- lab-setup1 # Run lab_setup with one or more --lab-setup-conf files
  from controller-0.
- unlock-controller-0 # Unlock controller-0 and wait for it to reboot.
- lab-setup2 # Run lab_setup with one or more --lab-setup-conf files
from controller-0. 
...
The autoinstaller has a predefined set of chains. User can select form
these chains and chose from which stage to which stage to do the install.
For example, if the user already executed config_controller it can chose
to continue from rsync-config to lab-setup2.

User can also create a custom set of chains, as he sees fit by specifying
them in the desired order. This allows better customization of the install
process. For example, an user may want to execute its own script after
config_controller, in this case he will have to specify a chain like this:
create-lab, install-controller-0, config-controller, rsync-config,
custom-script1, lab-setup1, unlock-controller-0, lab-setup2]

The installer supports creating virtualbox snapshots after each stage so
that in case of errors you don't need to reinstall from scratch. You can
restore the snapshot of the previous stage, wither retry or fix the issue
...
Configuration:
- Specify the number of nodes you want for your setup (1 or 2 controllers,
  x storages, y computes)
- Specify the number of disks attached to each node (the use the
  default sizes configured) or you can explicitly specify the sizes of the disks
...
- Ability to run your own custom scripts durring the install process
- Ability to define what scripts to be executed during custom script
  stages, their timeout, executed over ssh or serial, as normal user or as
  root. 

Other features
- Log files per lab & date.
- Enable hostiocache option for virtualbox VMs storage controllers
  to speed up install
- Basic support for Kubernates (AIO-SX installable through a custom
  chain)
...
- Install Virtualbox (recommend v5.1.x, had problems with 5.2, use at
  your own risk)
- Configure at least a vbox hostonly adapter network. If you want to
  use NAT also configue a NAT Network.
- Make sure you have rsync, ssh-keygen, sshpass commands installed.
...
/home/myousaf/pybox/configs/aio-sx/TiS_config.ini_centos --vboxnet-name
