...
def setup_networking(stream, ctrlr0_ip, gateway_ip, password='Li69nux*'):
    ...
    serial.send_bytes(
        stream,
        "sudo /sbin/ip addr add {}/24 dev {}".format(
            ip,
            interface),
        expect_prompt=False)
...
def install_controller_0(cont0_stream, setup_type, securityprofile, lowlatency,
    ...
    try:
        ...
        LOG.info(
            "So ignore the exception and wait for controller-0 to be installed again.")
...
def delete_lab(labname, force=False):
    ...
    if len(node_list) != 0:
        if not force:
            LOG.info(
                "This will delete lab {} with vms: {}".format(labname, node_list))
...
def get_disk_sizes(comma_list):
    ...
    for size in sizes:
        try:
            ...
            LOG.info(
                "Disk sizes must be a comma separated list of positive integers.")
            raise (
                "Disk sizes must be a comma separated list of positive integers.")
    return sizes
...
def create_lab(vboxoptions):
    ...
    if vboxoptions.workers:
        for id in range(0, vboxoptions.workers):
            node_name = vboxoptions.labname + "-worker-{}".format(id)
            nodes_list.append(node_name)
                ...
                for adapter in item.keys():
                    if adapter.isdigit():
                        ...
                        vboxmanage.vboxmanage_modifyvm(node,
                                                       nic=data['nic'], nictype=data[
                                                           'nictype'],
                                                       nicpromisc=data[
                                                       'nicpromisc'],
                                                       nicnum=int(adapter), intnet=data['intnet'],
                                                       hostonlyadapter=data[
                                                       'hostonlyadapter'],
                                                       natnetwork=data[
                                                       'natnetwork'],
...
def get_hostnames(
        ignore=None, personalities=['controller', 'storage', 'worker']):
    hostnames = {}
...
def wait_for_hosts(ssh_client, hostnames, status,
    ...
    while hostnames:
        ...
        if (time.time() - start) > HostTimeout.HOST_INSTALL:
            LOG.info(
                "VMs not booted in {}, aborting: {}".format(timeout, hostnames))
        ...
        if hostnames:
            LOG.info(
                "Waiting {}s before re-checking host status.".format(interval))
            time.sleep(interval)
...
def set_serial_prompt_mode(stream, mode):
    ...
    if serial_console_mode == mode:
        LOG.info(
            "Serial console prompt already set to '{}' mode.".format(mode))
        return
    if serial_console_mode != CONSOLE_USER_MODE:
        ...
        if serial.expect_bytes(stream, "ogin:", fail_ok=True, timeout=4):
            ...
            if serial.expect_bytes(stream, "ogin:", fail_ok=True, timeout=4):
                ...
                raise Exception(
                    "Failure getting login prompt on serial console!")
        serial.send_bytes(
            stream,
    ...
    def real_decorator(func):
        def func_wrapper(*args, **kwargs):
            try:
                ...
                LOG.info(
                    "Serial console login as '{}' failed. Retrying once.".format(mode))
                set_serial_prompt_mode(kwargs['stream'], mode)
...
def stage_config_controller(stream):
    ...
    if True:
        # Updated config file
        LOG.info(
            "#### Updating config_controller ini file networking settings and uploading it to controller.")
        destination = "/home/" + \
            vboxoptions.username + "/TiS_config.ini_centos"
        ...
        if vboxoptions.setup_type == AIO_SX:
            ...
            LOG.info(
                "Replacing OAM_NETWORK/IP_ADDRESS       from {}    to {}".format(
                    old_ip_address,
                    new_ip_address))
            ...
            LOG.info(
                "Replacing OAM_NETWORK/IP_START_ADDRESS from {}    to {}".format(
                    old_start_addr,
                    new_start_addr))
            ...
            LOG.info(
                "Replacing OAM_NETWORK/IP_END_ADDRESS   from {}    to {}".format(
                    old_end_addr,
                    new_end_addr))
...
def stage_unlock_controller0_serial(stream):
    ...
    # host_helper.login(stream, username=vboxoptions.username,
...
def stage_install_nodes(ssh_client):
    ...
    # Apply host-bulk-add
    _, _, exitcode = run_ssh_cmd(ssh_client,
                                 'source /etc/nova/openrc; system host-bulk-add {}'.format(
                                     destination),
                                 timeout=60)
    if exitcode != 0:
        # TODO: Raise bug, this never happens!
        msg = "Host bulk add failed, expecting exit code of 0 but got {}.".format(
            exitcode)
        LOG.info(msg)
        raise Exception(msg)
...
def run_custom_script(script, timeout, console, mode):
    ...
    if console == 'ssh':
        ...
        if return_code != 0:
            LOG.info(
                "Custom script '{}' return code is not 0. Aborting.".format(script))
            raise Exception(
                "Script execution failed with return code: {}".format(return_code))
    else:
        ...
        try:
            ...
            LOG.info(
                "Script execution completed. Checking if return code is 0.")
            serial.send_bytes(
                stream, "echo \"Return code: [$?]\"".format(script),
                timeout=3, prompt='Return code: [0]')
        finally:
            sock.close()
...
def get_custom_script_options(options_list):
    ...
    for c in not_allowed:
        if c in options_list:
            LOG.info(
                "Char '{}' not allowed in options list: {}.".format(c, options_list))
...
STAGE_CALLBACKS = {
    ...
    STG_LAB_SETUP4: {CALLBACK: stage_lab_setup4,
                     HELP: "Run lab_setup with one or more --lab-setup-conf files from controller-0."},
    STG_UNLOCK_WORKERS: {CALLBACK: stage_unlock_workers,
...
def load_config():
    ...
    # TODO: Validate input parameters (stages, config files presence, iso
    # location etc.)
...
if __name__ == "__main__":
    ...
    LOG.info("Executing {} stage(s): {}.".format(
        len(install_stages), install_stages))
