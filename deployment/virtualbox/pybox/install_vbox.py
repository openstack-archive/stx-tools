...
from utils.install_log import init_logging, get_log_dir
...
def menu_selector(stream, setup_type, securityprofile, lowlatency, install_mode='serial'):
    """
...
def setup_networking(stream, ctrlr0_ip, gateway_ip, password='Li69nux*'):
    ...
    host_helper.check_password(stream, password=password)
    time.sleep(2)
    serial.send_bytes(stream, "sudo route add default gw {}".format(gateway_ip), expect_prompt=False)
    host_helper.check_password(stream, password=password)

    if vboxoptions.vboxnet_type == 'hostonly':
        LOG.info("Pinging controller-0 at: {}...".format(ip))
        ...
        while tmout:
            ...
            raise ("Failed to establish connection in {}s "
                   "to controller-0 at: {}!".format(HostTimeout.NETWORKING_OPERATIONAL, ip))
...
def fix_networking(stream, release, password='Li69nux*'):
    ...
    serial.send_bytes(stream, "sudo /sbin/ip link set {} down".format(interface), expect_prompt=False)
    host_helper.check_password(stream, password=password)
    time.sleep(1)
    serial.send_bytes(stream, "sudo /sbin/ip link set {} up".format(interface), expect_prompt=False)
    host_helper.check_password(stream, password=password)
...
def install_controller_0(cont0_stream, setup_type, securityprofile, lowlatency,
                         install_mode, ctrlr0_ip, gateway_ip, username='wrsroot', password='Li69nux*'):
    """
    Installation of controller-0.
    """
    LOG.info("Starting installation of controller-0")
    start_time=time.time()
    menu_selector(cont0_stream, setup_type, securityprofile, lowlatency, install_mode)
...
def delete_lab(labname, force=False):
    ...
    if len(node_list) != 0:
        if not force:
            LOG.info("This will delete lab {} with vms: {}".format(labname, node_list))
            ...
            while True:
                ...
                if choice == 'y':
                    ...
                    LOG.info("Aborting!")
                    exit(1)
        LOG.info("#### Deleting lab {}.".format(labname))
        LOG.info("VMs in lab: {}.".format(node_list))
...
def get_disk_sizes(comma_list):
    ...
    for size in sizes:
        try:
            ...
            raise ("Disk sizes must be a comma separated list of positive integers.")
    return sizes
...
def create_lab(vboxoptions):
    ...
    if vboxoptions.storages:
        for id in range(0, vboxoptions.storages):
    ...
    for node in nodes_list:
        ...
        if "controller" in node:
            ...
            node_type = "compute"
            if vboxoptions.compute_disk_sizes:
                disk_sizes = get_disk_sizes(vboxoptions.compute_disk_sizes)
            else:
                no_disks = vboxoptions.compute_disks
                    ...
                    if adapter.isdigit():
                        ...
                        if vboxoptions.vboxnet_name is not 'none' and data['nic'] is 'hostonly':
                            if vboxoptions.vboxnet_type == 'nat':
                                ...
                                data['hostonlyadapter'] = vboxoptions.vboxnet_name
    ...
    vboxmanage.vboxmanage_storagectl(ctrlr0, storectl="ide", hostiocache=vboxoptions.hostiocache)
    vboxmanage.vboxmanage_storageattach(ctrlr0, storectl="ide", storetype="dvddrive",
                                        disk=vboxoptions.iso_location, device_num="0", port_num="1")


def get_hostnames(ignore=None, personalities=['controller', 'storage', 'compute']):
    hostnames = {}
    ...
    if vboxoptions.computes and 'compute' in personalities:
        for id in range(0, vboxoptions.computes):
            node_name = vboxoptions.labname + "-compute-{}".format(id)
            if ignore and node_name in ignore:
                continue
            hostnames[node_name] = 'compute-{}'.format(id)
    if vboxoptions.storages and 'storage' in personalities:
        for id in range(0, vboxoptions.storages):
            node_name = vboxoptions.labname + "-storage-{}".format(id)
            if ignore and node_name in ignore:
                continue
            hostnames[node_name] = 'storage-{}'.format(id)
...
def wait_for_hosts(ssh_client, hostnames, status, timeout=HostTimeout.HOST_INSTALL, interval=20):
    start = time.time()
    while hostnames:
        LOG.info("Hosts not {}: {}".format(status, hostnames))
        if (time.time() - start) > HostTimeout.HOST_INSTALL:
            LOG.info("VMs not booted in {}, aborting: {}".format(timeout, hostnames))
            raise Exception("VMs failed to go {}!".format(status)) 
        # Get host list
        host_statuses, _, _ = run_ssh_cmd(ssh_client, 'source /etc/nova/openrc; system host-list', timeout=30)
        ...
        if hostnames:
            LOG.info("Waiting {}s before re-checking host status.".format(interval))
            time.sleep(interval)
...
def run_ssh_cmd(ssh_client, cmd, timeout=5, log_output=True, mode=CONSOLE_USER_MODE):
    if mode == CONSOLE_ROOT_MODE:
        LOG.info(">>>>>")
        cmd = "sudo {}".format(cmd)
    LOG.info("#### Running command over ssh: '{}'".format(cmd))
    ...
    while True:
        ...
        if log_output and stdout:
            LOG.info("|{}".format(stdout_lines[-1]))
    stderr_lines = stderr.readlines()
    if log_output and stderr_lines:
        LOG.info("stderr:|\n{}".format("".join(stderr_lines)))
...
def set_serial_prompt_mode(stream, mode):
    ...
    if serial_console_mode == mode:
        LOG.info("Serial console prompt already set to '{}' mode.".format(mode))
        return
    if serial_console_mode != CONSOLE_USER_MODE:
        ...
        serial.send_bytes(stream, vboxoptions.username, prompt="assword:", timeout=30)
    ...
    if mode == 'root' and serial_console_mode != 'root':
        ...
        serial.send_bytes(stream, "cd /home/wrsroot", prompt="/home/wrsroot# ", timeout=30)
        serial.send_bytes(stream, "source /etc/nova/openrc\n",
                          timeout=30, prompt='keystone')
    ...
    def real_decorator(func):
        def func_wrapper(*args, **kwargs):
            try:
                ...
                LOG.info("Serial console login as '{}' failed. Retrying once.".format(mode))
                set_serial_prompt_mode(kwargs['stream'], mode)
...
def _connect_to_ssh():
    ...
    if vboxoptions.vboxnet_type == 'nat':
        ...
        keygen_arg = ip
    cmd = 'ssh-keygen -f "/home/{}/.ssh/known_hosts" -R {}'.format(getpass.getuser(), keygen_arg)
    LOG.info("CMD: {}".format(cmd))
    process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
    for line in iter(process.stdout.readline, b''):
        LOG.info("{}".format(line.decode("utf-8").strip()))
    process.wait()
...
def stage_install_controller0():
    ...
    LOG.info("Found nodes: {}".format(node_list))
    ...
    install_controller_0(cont0_stream, vboxoptions.setup_type, vboxoptions.securityprofile,
...
def stage_config_controller(stream):
    ip, port = get_ssh_ip_and_port('controller-0')  # Floating ip is not yet configured
    if True:
        # Updated config file
        LOG.info("#### Updating config_controller ini file networking settings and uploading it to controller.")
        ...
        LOG.info("Replacing OAM_NETWORK/CIDR             from {} to {}".format(old_cidr, new_cidr))
        configini['OAM_NETWORK']['CIDR'] = new_cidr
        old_gateway = configini['OAM_NETWORK']['GATEWAY']
        new_gateway = vboxoptions.vboxnet_ip
        LOG.info("Replacing OAM_NETWORK/GATEWAY          from {}    to {}".format(old_gateway, new_gateway))
        ...
        if vboxoptions.setup_type == AIO_SX:
            ...
            LOG.info("Replacing OAM_NETWORK/IP_ADDRESS       from {}    to {}".format(old_ip_address, new_ip_address))
            ...
            LOG.info("Replacing OAM_NETWORK/IP_START_ADDRESS from {}    to {}".format(old_start_addr, new_start_addr))
            ...
            LOG.info("Replacing OAM_NETWORK/IP_END_ADDRESS   from {}    to {}".format(old_end_addr, new_end_addr))
            configini['OAM_NETWORK']['IP_END_ADDRESS'] = new_end_addr
        
        #if vboxoptions.setup_type in [AIO_SX, AIO_DX]:
        #    old_system_mode = configini['SYSTEM']['SYSTEM_MODE']
        #    if vboxoptions.setup_type == AIO_DX:
        #        new_system_mode = 'duplex-direct'
        #    elif vboxoptions.setup_type == AIO_SX:
        #        new_system_mode = 'simplex'

        #    LOG.info("Replacing SYSTEM/SYSTEM_MODE           from '{}'  to '{}'".format(old_system_mode,
        #                                                                                new_system_mode))
        #    configini['SYSTEM']['SYSTEM_MODE'] = new_system_mode
        ...
        with tempfile.NamedTemporaryFile(mode='w') as fp:
            ...
            sftp_send(fp.name, remote_host=ip, remote_port=port, destination=destination,
                      username=vboxoptions.username, password=vboxoptions.password)
    else:
        destination = "/home/" + vboxoptions.username + "/TiS_config.ini_centos"
        sftp_send(vboxoptions.config_controller_ini, remote_host=ip, remote_port=port,
    ...
    install_lab.config_controller(stream, config_file=destination,
                                  password=vboxoptions.password, kubernetes=vboxoptions.kubernetes)
...
def stage_rsync_config():
    ...
    #LOG.info("Make scripts executable, in case any exists and we forgot to do it.")
    #serial.send_bytes(stream, "chmod +x *.sh 2>/dev/null; chmod +x *.py 2>/dev/null",
    #                  prompt='keystone', timeout=5)
...
def _run_lab_setup_serial(stream):
    ...
    #serial.send_bytes(stream, "source /etc/nova/openrc", prompt='keystone', timeout=30)
...
def _run_lab_setup(ssh_client):
    ...
    if exitcode != 0:
        msg = "Lab setup failed, expecting exit code of 0 but got {}.".format(exitcode)
        LOG.info(msg)
        raise Exception(msg)
...
def stage_unlock_controller0_serial(stream):
    ...
    #host_helper.login(stream, username=vboxoptions.username, password=vboxoptions.password)
...
def stage_install_nodes(ssh_client):
    ...
    # Apply host-bulk-add
    _, _, exitcode = run_ssh_cmd(ssh_client,
                      'source /etc/nova/openrc; system host-bulk-add {}'.format(destination),
                      timeout=60)
    if exitcode != 0:
        # TODO: Raise bug, this never happens!
        msg = "Host bulk add failed, expecting exit code of 0 but got {}.".format(exitcode)
        LOG.info(msg)
        raise Exception(msg)

    # Start hosts one by one, wait 10s between each start
    vms = vboxmanage.get_all_vms(vboxoptions.labname, option="vms")
    runningvms = vboxmanage.get_all_vms(vboxoptions.labname, option="runningvms")
    powered_off = list(set(vms) - set(runningvms))
    LOG.info("#### Powered off VMs: {}".format(powered_off))
    for vm in powered_off:
        LOG.info("#### Powering on VM: {}".format(vm))
...
def stage_unlock_controller1(ssh_client):
    ...
    run_ssh_cmd(ssh_client, 'source /etc/nova/openrc; system host-unlock controller-1', timeout=60)
...
def stage_unlock_storages(ssh_client):
    ...
    for storage in storages:
        run_ssh_cmd(ssh_client, 'source /etc/nova/openrc; system host-unlock {}'.format(storage), timeout=60)
...
@connect_to_ssh
def stage_unlock_computes(ssh_client):
    # Unlock all, wait for all hosts, except ctrl0 to be 'available'
    computes = list(get_hostnames(personalities=['compute']).values())
    ctrl0 = vboxoptions.labname + '-controller-0'

    for compute in computes:
        run_ssh_cmd(ssh_client, 'source /etc/nova/openrc; system host-unlock {}'.format(compute), timeout=60)
        LOG.info("Waiting 15s before next unlock")
        time.sleep(15)
...
def run_custom_script(script, timeout, console, mode):
    LOG.info("#### Running custom script {} with options:".format(script))
    LOG.info("     timeout:        {}".format(timeout))
    LOG.info("     console mode:   {}".format(console))
    LOG.info("     user mode: {}".format(mode))
    if console == 'ssh':
        ssh_client = _connect_to_ssh()
        _, __, return_code = run_ssh_cmd(ssh_client, "./{}".format(script),
                                         timeout=timeout, mode=mode)
        if return_code != 0: 
            LOG.info("Custom script '{}' return code is not 0. Aborting.".format(script))
        ...
        try:
            ...
            serial.send_bytes(stream, "echo \"Return code: [$?]\"".format(script),
                              timeout=3, prompt='Return code: [0]')
        finally:
            sock.close()

def get_custom_script_options(options_list):
    LOG.info("Parsing custom script options: {}".format(options_list))
    # defaults
    script = ""
    timeout = 5
    console = 'serial'
    ...
    for c in not_allowed:
        if c in options_list:
            LOG.info("Char '{}' not allowed in options list: {}.".format(c, options_list))
    ...
    if len(options) >= 3:
        ...
        if console not in CONSOLES:
            raise "Console must be one of {}, not {}.".format(CONSOLES, console)
    if len(options) >= 4:
        mode = options[3]
...
def stage_custom_script1():
    if vboxoptions.script1:
        script, timeout, console, mode = get_custom_script_options(vboxoptions.script1)
    else:
...
def stage_custom_script2():
    if vboxoptions.script2:
        script, timeout, console, mode = get_custom_script_options(vboxoptions.script2)
    else:
...
def stage_custom_script3():
    if vboxoptions.script3:
        script, timeout, console, mode = get_custom_script_options(vboxoptions.script3)
    else:
...
def stage_custom_script4():
    if vboxoptions.script4:
        script, timeout, console, mode = get_custom_script_options(vboxoptions.script4)
    else:
...
def stage_custom_script5():
    if vboxoptions.script5:
        script, timeout, console, mode = get_custom_script_options(vboxoptions.script5)
    else:
...
STG_UNLOCK_COMPUTES = "unlock-computes"
...
STAGE_CALLBACKS = {
    STG_CREATE_LAB: {CALLBACK: stage_create_lab,
                     HELP: "Create VMs in vbox: controller-0, controller-1..."},
    STG_INSTALL_CONTROLLER0: { CALLBACK: stage_install_controller0,
                              HELP: "Install controller-0 from --iso-location"},
    STG_CONFIG_CONTROLLER: {CALLBACK: stage_config_controller,
                            HELP: "Run config controller using the --config-controller-ini updated based on --ini-* options."},
    STG_RSYNC_CONFIG: {CALLBACK: stage_rsync_config,
                       HELP: "Rsync all files from --config-files-dir and --config-files-dir* to /home/wrsroot."},
    STG_LAB_SETUP1: {CALLBACK: stage_lab_setup1,
                     HELP: "Run lab_setup with one or more --lab-setup-conf files from controller-0."},
    STG_UNLOCK_CONTROLLER0: {CALLBACK: stage_unlock_controller0,
                             HELP: "Unlock controller-0 and wait for it to reboot."},
    STG_LAB_SETUP2: {CALLBACK: stage_lab_setup2,
                     HELP: "Run lab_setup with one or more --lab-setup-conf files from controller-0."},
    STG_INSTALL_NODES: {CALLBACK: stage_install_nodes,
                        HELP: "Generate a host-bulk-add.xml, apply it and install all other nodes, wait for them to be 'online."},
    STG_UNLOCK_CONTROLLER1: {CALLBACK: stage_unlock_controller1,
                             HELP: "Unlock controller-1, wait for it to be 'available'"},
    STG_LAB_SETUP3: {CALLBACK: stage_lab_setup3,
                     HELP: "Run lab_setup with one or more --lab-setup-conf files from controller-0."},
    STG_UNLOCK_STORAGES: {CALLBACK: stage_unlock_storages,
                          HELP: "Unlock all storage nodes, wait for them to be 'available'"},
    STG_LAB_SETUP4: {CALLBACK: stage_lab_setup4,
                     HELP: "Run lab_setup with one or more --lab-setup-conf files from controller-0."},
    STG_UNLOCK_COMPUTES: {CALLBACK: stage_unlock_computes,
                          HELP: "Unlock all computes, wait for them to be 'available"},
    STG_LAB_SETUP5: {CALLBACK: stage_lab_setup5,
                     HELP: "Run lab_setup with one or more --lab-setup-conf files from controller-0."},
    STG_CUSTOM_SCRIPT1: {CALLBACK: stage_custom_script1,
                         HELP: "Run a custom script from /home/wrsroot, make sure you upload it in the rsync-config stage and it is +x. See help."},
    STG_CUSTOM_SCRIPT2: {CALLBACK: stage_custom_script2,
                         HELP: "Run a custom script from /home/wrsroot, make sure you upload it in the rsync-config stage and it is +x. See help."},
    STG_CUSTOM_SCRIPT3: {CALLBACK: stage_custom_script3,
                         HELP: "Run a custom script from /home/wrsroot, make sure you upload it in the rsync-config stage and it is +x. See help."},
    STG_CUSTOM_SCRIPT4: {CALLBACK: stage_custom_script4,
                         HELP: "Run a custom script from /home/wrsroot, make sure you upload it in the rsync-config stage and it is +x. See help."},
    STG_CUSTOM_SCRIPT5: {CALLBACK: stage_custom_script5,
                         HELP: "Run a custom script from /home/wrsroot, make sure you upload it in the rsync-config stage and it is +x. See help."},
    # internal testing
...
def load_config():
    ...
    # TODO: Validate input parameters (stages, config files presence, iso location etc.)

def pre_validate(vboxoptions):
    if vboxoptions.skype_notification_test:
        # Nothing to validate
        return
    err = False
...
def validate(vboxoptions, stages):
    ...
    if vboxoptions.vboxnet_type == 'nat':
        ...
        if vboxoptions.controllers > 1 and not vboxoptions.nat_controller1_local_ssh_port:
            ...
            err = True
        # TODO: Check that ports are still open? maybe after creating the setup
    else:
        if vboxoptions.setup_type != AIO_SX:
...
def send_skype_status(success=None, exception=None, prev_stage=None, next_stage=None):
    LOG.info("Sending Skype message with install status "
             "to: {}.".format(vboxoptions.skype_destination_user))
    # Connect to skype
    from utils import skype  # Local import, skype is experimental
    skype.init(vboxoptions.skype_username,
               vboxoptions.skype_password,
               vboxoptions.skype_destination_user)
    # Send general status
    msg = ""
    if success == True:
        msg = ("Install of {} on {}@{} SUCCEEDED! :)\n"
               "".format(vboxoptions.labname, socket.gethostname(), getpass.getuser()))
    elif success == False:
        msg = ("Install of {} on {}@{} FAILED! :|\n"
               "".format(vboxoptions.labname, socket.gethostname(), getpass.getuser()))
    if prev_stage and next_stage:
        msg = ("Install of {} on {}@{} completed stage '{}', continuing with stage '{}'.\n"
               "".format(vboxoptions.labname, socket.gethostname(), getpass.getuser(),
                         prev_stage, next_stage))
    if msg:
        skype.send_msg(msg)
    # Send metrics
    msg = kpi.get_kpi_metrics_str()
    skype.send_msg(msg)
    # Send exception details
    if exception:
        skype.send_msg(exception)


def wrap_stage_help(stage, help, number=None):
    if number:
...
if __name__ == "__main__":    
    ...
    if vboxoptions.skype_notification_test:
        # This is only a Skype test, no install
        if not vboxoptions.skype_destination_user:
            print("Where do you want to send a 'test' messgae?. "
                  "Please set --skype-destination-user.")
            exit(1)
        print("==== Testing Skype connectivity ====")
        print("Importing library")
        from utils import skype
        ...
        skype.send_msg("Test!")
        print("Message sent, check skype!")
        print("==== Skype connectivity test done ====")
        exit(0)
    
    pre_validate(vboxoptions)
    
    init_logging(vboxoptions.labname, vboxoptions.logpath)
    LOG.info("Logging to directory: {}".format(get_log_dir() + "/"))
    
    LOG.info("Install manages: %s controllers, %s computes, %s storages."
             % (vboxoptions.controllers, vboxoptions.computes, vboxoptions.storages))
    ...
    if vboxoptions.custom_stages:
        ...
        for stage in install_stages:
            ...
            if invalid_stages:
                LOG.info("Following custom stages are not supported: {}.\n"
                         "Choose from: {}".format(invalid_stages, AVAILABLE_STAGES))
                exit(1)
    else:
    ...
    LOG.info("Executing {} stage(s): {}.".format(len(install_stages), install_stages))    
    
    validate(vboxoptions, install_stages)
    
    if vboxoptions.skype_notify_beginning:
        from utils import skype
        if not vboxoptions.skype_destination_user:
            LOG.info("Where do you want to send notifications?. "
                     "Please set --skype-destination-user.")
            exit(1)
        LOG.info("Send Skype notification that we are starting.")
        msg = ("Starting install of {} on {}@{} (fingers)! \n"
               "".format(vboxoptions.labname, socket.gethostname(), getpass.getuser()))
        skype.init(vboxoptions.skype_username,
                   vboxoptions.skype_password,
                   vboxoptions.skype_destination_user)
    ...
    for stage in install_stages:
        ...
        start = time.time()
        try:
            # Send skype notification for stage
            try:
                if (prev_stage and vboxoptions.skype_destination_user and
                        vboxoptions.skype_notify_each_stage):
                    send_skype_status(prev_stage=prev_stage, next_stage=stage)
            except:
                LOG.info("Sending Skype message failed, continuing.")
                
            LOG.info("######## ({}/{}) Entering stage {} ########".format(stg_no, len(install_stages), stage))
            STAGE_CALLBACKS[stage][CALLBACK]()

            # Take snapshot if configured
            if vboxoptions.snapshot:
                vboxmanage.take_snapshot(vboxoptions.labname, "snapshot-AFTER-{}".format(stage))
            ...
            kpi.print_kpi_metrics()
            LOG.info("Exception details: {}".format(e))
            if vboxoptions.skype_destination_user:
                send_skype_status(success=False, exception=traceback.format_exc())
            raise
        # Stage completed
        prev_stage = stage 
    ...
    kpi.print_kpi_metrics()
    if vboxoptions.skype_destination_user:
        send_skype_status(success=True)
