...
def connect(hostname, port=10000, prefix=""):
    ...
    if 'controller-0'in hostname:
        socketname += '_serial'
    LOG.info("Connecting to {} at {}".format(hostname, socketname))
...
def get_output(stream, cmd, prompts=None, timeout=5, log=True, as_lines=True, flush=True):
    ...
    if flush:
        try:
            ...
            if trash:
                try:
                    LOG.info("Buffer has bytes before cmd execution: {}".format(trash.decode('utf-8')))
                except Exception:
                    pass
...
def send_bytes(stream, text, fail_ok=False, expect_prompt=True, prompt=None, timeout=180, send=True, flush=True):
    """
