...
def sftp_send(source, remote_host, remote_port, destination, username, password):
    ...
    while retry < 8:
        try:
            ssh_client.connect(remote_host, port=remote_port, username=username, password=password, look_for_keys=False, allow_agent=False)
...
def send_dir(source, remote_host, remote_port, destination, username, password, follow_links=True, clear_known_hosts=True):
    ...
    LOG.info("Running rsync of dir: {source} -> {username}@{remote_host}:{destination}".format(**params))
