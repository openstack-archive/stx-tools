...
def handle_args():
    ...
    ******************************************
    ...
    parser.add_argument("--config-files-dir", help=
        """
        Directory with config files, scripts, images (i.e. lab_setup.sh, lab_setup.conf, license.lic, ...)
        that are needed for the install. All files at this location are transfered to controller-0 in
        /home/wrsroot. You can add you own scripts that you need to be present on the controller.
        Caution: rsync will follow links and will fail if links are broken! Use --config-files-dir-dont-follow-links
        instead. Also, you can use both options for different folders.
        """,
        type=str)
    ...
    parser.add_argument("--config-controller-ini", help=
        """
        Path to the local config_controller .ini. This file is transfered to the controller.
        NOTE: OAM configuration in this ini is updated dynamically based on networking related args.
        (e.g. TiS_config.ini_centos, ~/TiS_config.ini_centos, /folk/cgts...).
        """,
        type=str)
    ...
    **************************************
    ...
    parser.add_argument("--storage-disks", help=
        ...
        """,
        type=int, default=3, choices=[1,2,3,4,5,6,7])
    parser.add_argument("--compute-disks", help=
        """
        Select the number of disks for a compute VM. default is 2
        """,
    ...
    parser.add_argument("--storage-disk-sizes", help=
        ...
        """,
        type=str)
    parser.add_argument("--compute-disk-sizes", help=
        """
        Configure size in MiB of compute disks as a comma separated list.
        """,
        type=str)
    """
    **************
    ...
    **************
    ...
    parser.add_argument("--controller0-ip", help=
        ...
        update IP_ADDRESS in [OAM_NETWORK] of config_controller config file of an AIO SX setup.
    ...
    parser.add_argument("--nat-controller-floating-local-ssh-port", help=
        """
        When oam network is configured as 'nat' a port on the vbox host is used for connecting to ssh on floating controller.
        No default value is configured. This is mandatory if --vboxnet-type is 'nat' for non AIO-SX deployments.
        """,
        type=str)
    parser.add_argument("--nat-controller0-local-ssh-port", help=
        """
        When oam network is configured as 'nat' a port on the vbox host is used for connecting to ssh on controller-0.
        This is mandatory if --vboxnet-type is 'nat'. No default value is configured.
        """,
        type=str)
    parser.add_argument("--nat-controller1-local-ssh-port", help=
        """
        When oam network is configured as 'nat' a port on the vbox host is used for connecting to ssh on controller-1.
        No default value is configued. This is mandatory if --vboxnet-type is 'nat' for non AIO-SX deployments or
        if second controller is installed.
        """,
        type=str)
    parser.add_argument("--ini-oam-cidr", help=
        """
        The IP network and mask for the oam net, used to update CIDR value in [OAM_NETWORK]
        of config_controller config file.
        Default is 10.10.10.0/24
        """,
        type=str)
    parser.add_argument("--ini-oam-ip-start-address", help=
        """
        The start for the oam net allocation, used to update IP_START_ADDRESS value in
        [OAM_NETWORK] of config_controller config file.
        Not needed for AIO SX setups.
        """,
        type=str)
    parser.add_argument("--ini-oam-ip-end-address", help=
        """
        The end for the oam net allocation, used to update IP_END_ADDRESS value in
        [OAM_NETWORK] of config_controller config file.
        Not needed for AIO SX setups.
        """,
        type=str)
    ...
    ******************
    ...
    parser.add_argument("--script1", help=
        """
        Name of an executable script file plus options. Has to be present
        in --config-files-dir.
        It will be transfered to host in rsync-config stage and executed
        as part of custom-script1 stage.
    ...
    **************************************
    """
    parser.add_argument("--skype-username", help=
        """
        User name of your Skype account.
        """,
        default='starlingx.bot@gmail.com',
        type=str)
    parser.add_argument("--skype-password", help=
        """
        Password of your Skype account.
        """,
        default='WindriverB0t',
        type=str)
    parser.add_argument("--skype-destination-user", help=
        """
        Destination user for messages.
        """,
        default=None,
        type=str)
    parser.add_argument("--skype-notify-beginning", help=
        """
        Send a notification at the beginning of the install.
        Helpful for early validation of skype connectivity.
        """,
        action='store_true')
    parser.add_argument("--skype-notify-each-stage", help=
        """
        Send a notification after each successful stage.
        """,
        action='store_true')
    parser.add_argument("--skype-notification-test", help=
        """
        Test Skype connectivity and send a notification
        """,
        action='store_true')
    """
    **************************************
    ...
    **************************************
    ...
    parser.add_argument("--userid", help=
        """
        Unique user id to differentiate vbox machine unique names such as interface names or serial ports
        even if setups have the same names for different users. Default is your username on this machine.
        """,
        type=str,
    ...
    parser.add_argument("--hostiocache", help=
        ...
        """,
        action='store_true')
    parser.add_argument("--kubernetes", help=
        """
        Run config_controller with --kubernates option
        """,
        action='store_true')
    return parser
