...
get_vms_by_group () {
    local group=$1
    vms=$(VBoxManage list -l vms | awk -v group="/$group" '/^Name:/ { name = $2; } /^Groups:/ { groups = $2; } /^UUID:/ { uuid = $2; if (groups == group) print name, uuid; }')
    echo "###### VMs in group:" >&2
    echo "$vms" >&2
    echo "$vms"
...
if [[ "$SNAP_ACTIONS" = *"$ACTION"* ]]; then
    ...
    while read -r vm; do 
        vmA=(${vm})
        state=$(vboxmanage showvminfo ${vmA[1]} --machinereadable | grep "VMState=")
        if [[ "$state" = *"saved"* ]]; then
            if [ -z "$saved_vms" ]; then
                saved_vms="$vm"
            else
    ...
    if [ ! -z "$saved_vms" ]; then
        while read -r vm; do 
            ...
            (VBoxHeadless --start-paused --startvm ${vm[1]} --vrde config >/dev/null 2>&1) &
            sleep 1
            while true; do
                state=$(vboxmanage showvminfo ${vm[1]} --machinereadable | grep "VMState=")
