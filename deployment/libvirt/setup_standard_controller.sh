...
for i in {0..1}; do
    ...
    sed -i -e "s,NAME,${CONTROLLER_NODE}," \
           -e "s,DISK0,/var/lib/libvirt/images/${CONTROLLER_NODE}-0.img," \
           -e "s,DISK1,/var/lib/libvirt/images/${CONTROLLER_NODE}-1.img," \
           -e "s,%BR1%,${BRIDGE_INTERFACE}1," \
           -e "s,%BR2%,${BRIDGE_INTERFACE}2," \
           -e "s,%BR3%,${BRIDGE_INTERFACE}3," \
           -e "s,%BR4%,${BRIDGE_INTERFACE}4," \
           ${DOMAIN_FILE}
    if [ $i -eq 0 ]; then
...
for i in {0..1}; do
    ...
    sed -i -e "s,NAME,${COMPUTE_NODE}," \
           -e "s,DISK0,/var/lib/libvirt/images/${COMPUTE_NODE}-0.img," \
           -e "s,DISK1,/var/lib/libvirt/images/${COMPUTE_NODE}-1.img," \
           ${DOMAIN_FILE}
