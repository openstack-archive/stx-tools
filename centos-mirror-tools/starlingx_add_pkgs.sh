...
function feature_to_pkg()
{
    ...
    #
    local feature=$1
    local pkg=$($REPOQUERY_STD_CMD $feature | head -1)
    if [ -z $pkg ]; then
...
function pkg_in_cgcsrepo()
{
    ...
    local results=$($REPOQUERY_LOCAL_CMD --whatprovides $pkg)
    if [ -n "$results" ]; then
        return 0
    fi

    local pkgname=$($REPOQUERY_ALL_CMD --quiet $REPOCFG_ARGS --qf='%{name}' --whatprovides $pkg | head -1)
    if [ -z "$pkgname" ]; then
...
function download_pkg()
{
    ...
    #
    local feature=$1
    local pkg=$(feature_to_pkg $feature)
    if [ -z "$pkg" ]; then
