...
# tarballs and other files not dowloaded from a yum repository.
...
LOGFILE=/export/log/dl_update.log
...
dl_git_from_url () {
    ...
    if [ ! -d "$DL_DIR" ]; then
        ...
        CMD="mv hooks/post-update.sample hooks/post-update"
        echo "$CMD"
        eval $CMD
        if [ $? -ne 0 ]; then
            echo "Error: $CMD"
...
raw_dl_from_non_rpm_lst () {
    ...
    grep -v '^#' $FILE | while IFS='#' read -r TAR DIR URL METHOD UTIL SCRIPT; do
        ...
        echo "Processing: TAR=$TAR  DIR=$DIR  URL=$URL  METHOD=$METHOD  UTIL=$UTIL"
        TARBALL_NAME="${TAR//!/}"
        if [[ "$TAR" =~ ^'!' ]]; then
            case $METHOD in
                http|http_script)
