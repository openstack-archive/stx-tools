...
get_repoquery_info() {
    ...
    if [ "$_arch" == "x86_64" ]; then
        ...
        _repoquery=$(repoquery --qf '%{NAME}-%{VERSION}-%{RELEASE}.%{ARCH}' --archlist=x86_64 "$_package_name")
    else
        _repoquery=$(repoquery --qf '%{NAME}-%{VERSION}-%{RELEASE}.%{ARCH}' "$_package_name")
    fi
    echo "$_repoquery"
}
...
info "Getting info for src packages..."
src_rpms=$(echo "$(grep "src.rpm" < $RPMS_CENTOS_LIST)")
check_rpms "$src_rpms"

info "Getting info for noarch packages..."
noarch_rpms=$(echo "$(grep "noarch.rpm" < $RPMS_CENTOS_LIST)")
check_rpms "$noarch_rpms"

info "Getting info for x86_64 packages..."
x86_64_rpms=$(echo "$(grep "x86_64.rpm" < $RPMS_CENTOS_LIST)")
check_rpms "$x86_64_rpms"
