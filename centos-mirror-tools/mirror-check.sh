...
RPMS_3RD_PARTY_LIST="rpms_from_centos_3rd_parties.lst"
...
get_repoquery_info() {
    ...
    repoquery -C --qf '%{NAME}-%{VERSION}-%{RELEASE}.%{ARCH}' $repoquery_opts "$_package_name"
}
...
for rpm_list in "$RPMS_CENTOS_LIST" "$RPMS_3RD_PARTY_LIST"; do
    ...
    for arch in "src" "noarch" "x86_64"; do
        ...
        rpms=$(echo "$(grep "$arch.rpm" < $rpm_list)")
        check_rpms "$rpms"
    done
done
