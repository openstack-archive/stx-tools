...
make_cengn_yum_conf="./make_cengn_yum_conf.sh"
...
if [ "$dl_flag" == "" ]; then
    ...
    if ! dl_from_stx; then
        ...
        TEMP_DIR=$(mktemp -d /tmp/cengn_XXXXXX)
        TEMP_CONF="$TEMP_DIR/yum.conf"
        need_file ${make_cengn_yum_conf}
        need_dir ${TEMP_DIR}
        if [ $use_system_yum_conf -eq 0 ]; then
            if dl_from_upstream; then
                ${make_cengn_yum_conf} -R -d $TEMP_DIR -y $alternate_yum_conf -r $alternate_repo_dir -D $distro
            else
                ${make_cengn_yum_conf} -d $TEMP_DIR -y $alternate_yum_conf -r $alternate_repo_dir -D $distro
            fi
            rpm_downloader_extra_args="${rpm_downloader_extra_args} -c $TEMP_CONF"
        else
            if dl_from_upstream; then
                ${make_cengn_yum_conf} -R -d $TEMP_DIR -y /etc/yum.conf -r /etc/yum.repos.d -D $distro
            else
                ${make_cengn_yum_conf} -d $TEMP_DIR -y /etc/yum.conf -r /etc/yum.repos.d -D $distro
            fi
            rpm_downloader_extra_args="${rpm_downloader_extra_args} -c $TEMP_CONF"
        fi
    fi
